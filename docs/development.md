...
packages/engine/     Game creation, reducer, events, IDs, and views
...
- Shared imports use workspace package names such as `@mtg-engine/engine` and `@mtg-engine/schemas`.
- Runtime validation belongs in `packages/schemas`.
- State transitions belong in `packages/engine/src/reducer.ts`.
...
2. Handle the command in `packages/engine/src/reducer.ts`.
